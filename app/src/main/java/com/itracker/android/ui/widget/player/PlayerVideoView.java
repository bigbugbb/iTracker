...
public class PlayerVideoView extends TextureView implements MediaController.MediaPlayerControl, TrackerPlayer {
    ...
    private VideoStateListener mPlayStateListener;

    private AlertDialog mErrorDialog;

    public PlayerVideoView(final Context context, final AttributeSet attrs) {
    ...
    public void onInitializeAccessibilityEvent(final AccessibilityEvent event) {
        ...
        event.setClassName(PlayerVideoView.class.getName());
    }

    @Override
    public void onInitializeAccessibilityNodeInfo(final AccessibilityNodeInfo info) {
        super.onInitializeAccessibilityNodeInfo(info);
        info.setClassName(PlayerVideoView.class.getName());
    }
    ...
    private boolean allowPlayStateToHandle(final int frameworkError) {
        if (frameworkError == MediaPlayer.MEDIA_ERROR_UNKNOWN || frameworkError == MediaPlayer.MEDIA_ERROR_IO) {
            ...
            if (hasPlayStateListener()) {
                return mPlayStateListener.onStopWithExternalError(mMediaPlayer.getCurrentPosition() / MILLIS_IN_SEC);
            }
        }
        return false;
    }
