 ...
 */
public class RemoteTrackerDataFetcher {
    ...
    private static final String URL_OVERRIDE_FILE_NAME = "show_manifest_url_override.txt";
    ...
    public RemoteTrackerDataFetcher(Context context) {
        mContext = context;
     ...
     */
    public String[] fetchCompanyDataIfNewer(String refTimestamp) throws IOException {
...
//        }
    ...
    private String getManifestUrl() {
        ...
        if (urlOverrideFile.exists()) {
            try {
                String overrideUrl = DataFileUtils.readFileAsString(urlOverrideFile).trim();
...
//        }
    ...
    private File getCacheFile(String url) {
        ...
        return new File(mContext.getCacheDir() + File.separator + CACHE_DIR + File.separator + cacheKey);
    }
    ...
    private String loadFromCache(String url) throws IOException {
        ...
        if (cacheFile.exists()) {
            ...
            return DataFileUtils.readFileAsString(cacheFile);
        } else {
    ...
    private void writeToCache(String url, String body) throws IOException {
        ...
        File cacheFile = getCacheFile(url);
        createCacheDir();
        DataFileUtils.writeFile(body, cacheFile);
    ...
    private String[] processManifest(String manifestJson) throws IOException {
        ...
        if (manifest.format == null || !manifest.format.equals(Config.MANIFEST_FORMAT)) {
        ...
        if (manifest.data == null || manifest.data.length == 0) {
            LOGW(TAG, "Manifest does not list any files. Nothing done.");
            return null;
        }

        LOGD(TAG, "Manifest lists " + manifest.data.length + " data files.");
        String[] jsons = new String[manifest.data.length];
        for (int i = 0; i < manifest.data.length; i++) {
            String url = manifest.data[i];
