...
public class CollectionView extends ListView {
    ...
    public CollectionView(Context context, AttributeSet attrs, int defStyle) {
        super(new ContextThemeWrapper(context, R.style.PhotoFastScrollbarTheme), attrs, defStyle);
    ...
    private int getRowViewType(int row) {
        ...
        if (computeRowContent(row, rowComputeResult)) {
            if (hasCustomGroupView()) {
                ...
                return BUILTIN_VIEWTYPE_COUNT + mInventory.getGroupIndex(rowComputeResult.groupId);
            }
        } else {
    ...
    private void setupLayoutParams(View view, RowComputeResult rowInfo, int columnIndex, int columnCount) {
        ...
        viewLayoutParams.leftMargin = columnIndex == 0 ? 0 : mInternalPadding / 2;
        viewLayoutParams.rightMargin = columnIndex == columnCount - 1 ? 0 : mInternalPadding / 2;
        viewLayoutParams.topMargin = rowInfo.groupOffset == 0 ? mInternalPadding : 0;
    ...
    private View makeNewItemRow(RowComputeResult rowInfo) {
        ...
        for (int i = 0; i < nbColumns; i++) {
            ...
            setupLayoutParams(view, rowInfo, i, nbColumns);
            ll.addView(view);
        }
    ...
    private View recycleItemRow(View convertView, RowComputeResult rowInfo) {
        ...
        for (int i = 0; i < nbColumns; i++) {
            ...
            if (view != newView) {
                setupLayoutParams(newView, rowInfo, i, nbColumns);
                ll.removeViewAt(i);
     ...
     * header.
     */
    public static class Inventory implements Iterable<InventoryGroup> {
        ...
        public InventoryGroup getGroup(int groupIndex) {
        ...
        @Override
        public Iterator<InventoryGroup> iterator() {
