 ...
 */
package com.localytics.android.itracker.sync.userdata.util;
...
import com.localytics.android.itracker.sync.userdata.UserAction;
...
public class UserActionHelper {
    ...
    static public void updateContentProvider(Context context, List<UserAction> userActions, String account) {
        ...
        try {
            context.getContentResolver().applyBatch(TrackerContract.CONTENT_AUTHORITY, batch);
    ...
    static private ContentProviderOperation createUpdateOperation(Context context, UserAction action, String account) {
        if (action.type == UserAction.TYPE.ADD_STAR) {
            return null;//ContentProviderOperation
//                    .newInsert(
//                            ShowContract.addOverrideAccountName(
//                                    ShowContract.MySchedule.CONTENT_URI, account))
//                    .withValue(ShowContract.MySchedule.MY_SCHEDULE_DIRTY_FLAG, "0")
//                    .withValue(ShowContract.MySchedule.BLOG_ID, action.sessionId)
//                    .build();
        } else {
            return null;//ContentProviderOperation
//                    .newDelete(
...
//                                    ShowContract.MySchedule.CONTENT_URI, account))
//                    .withSelection(
//                            ShowContract.MySchedule.BLOG_ID + " = ? AND " +
//                                    ShowContract.MySchedule.MY_SCHEDULE_ACCOUNT_NAME + " = ? ",
