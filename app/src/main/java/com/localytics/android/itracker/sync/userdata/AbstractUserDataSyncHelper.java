...
package com.localytics.android.itracker.sync.userdata;
...
public abstract class AbstractUserDataSyncHelper {
    ...
    public boolean sync() {
        ...
        Cursor scheduleData = null; //mContext.getContentResolver().query(
...
//            ServerUtils.notifyUserDataChanged(mContext);
        }
//        if (dataChanged) {
//            LOGD(TAG, "Notifying changes on paths related to user data on Content Resolver.");
//            ContentResolver resolver = mContext.getContentResolver();
//            for (String path : TrackerContract.USER_DATA_RELATED_PATHS) {
...
//                    .withSelection(MySchedule.BLOG_ID + "=? AND " +
...
//            ops.add(op);
        ...
        try {
            ContentProviderResult[] result = mContext.getContentResolver().applyBatch(TrackerContract.CONTENT_AUTHORITY, ops);
